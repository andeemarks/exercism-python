...
def letter_grades(highest):
    ...
    thresholds = range(LOWEST_PASSING_GRADE, highest,
                       (int)((highest + 1 - LOWEST_PASSING_GRADE) / NUMBER_OF_GRADES))
...
def student_ranking(student_scores, student_names):
    ...
    return [f"{number + 1}. {name}: {student_scores[number]}"
            for number, name in enumerate(student_names)]
...
def perfect_score(student_info):
    ...
    return perfect_scores[0] if len(perfect_scores) > 0 else []
